...
namespace MultiscaleModelling.Core
{
    class Simul
    {
        ...
        private MapControl _mapController;
        ...
        public MapControl MapController { get { return _mapController; } set { _mapController = value; } }
            ...
            for (int x = 1; x <= _configuration.Width; x++)
            {
                for (int y = 1; y <= _configuration.Height; y++)
                {
                    ...
                    if (_selectedGrains.Any(k => k.Id == node.Id))
                    {
                        ...
                        {
                            _mapController.SetNode(x, y, new Pix()
                            {
        ...
        public void Restart()
        {
            _mapController = new MapControl(_configuration.Width, _configuration.Height);
            _mapController.Commit();
            _endSimulation = false;
            _isMapEmpty = true;
        ...
        public void Initialize(Configuration config)
        {
            ...
            {
                _mapController = new MapControl(config.Width, config.Height);
                _mapController.Commit();
            }
            else
            {
        ...
        public List<Pix> GetListOfEdgesNodes()
        {
            List<Pix> edgesNodes = new List<Pix>();
        ...
        {
            List<Pix> grainNodes = new List<Pix>();
            List<Pix> grainNodesNew;
            for (int x = 1; x <= _configuration.Width; x++)
            {
                for (int y = 1; y <= _configuration.Height; y++)
                {
