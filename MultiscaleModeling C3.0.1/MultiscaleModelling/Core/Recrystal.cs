...
namespace MultiscaleModelling.Core
{
    class Recrystal : IProcessable
    {
        ...
        public Recrystal(Random random, Simul simulationEngine)
        {
            _random = random;
        ...
        private void GrowtSimulation()
        {
            ...
            List<Pix> nodesToRandom = new List<Pix>();
        ...
        private void ProcessNode(Pix node)
        {
        ...
        private double GetEnrgy(int id, int x, int y, List<Pix> neighbourhoods)
        {
        ...
        private void SeedNewNode(int amount)
        {
            ...
            List<Pix> nodesToRecrystalization = new List<Pix>();
            for (int x = 1; x <= _simulationEngine.Configuration.Width; x++)
            {
                for (int y = 1; y <= _simulationEngine.Configuration.Height; y++)
                {
